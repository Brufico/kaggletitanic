...
tdf$Deck <- factor(ifelse( tdf$Cabin != "", substr(tdf$Cabin,start=1,stop = 1), "Unknown"))
...
ggplot(tdf[tdf$Deck != "Unknown",] ) +
        geom_bar(aes(Deck, y=..prop.., fill = Pclass, group = 1)) + 
        facet_grid(Pclass ~ .)    
```
...
```{r survival-by5, w = w.12, a=a.43, fig.cap="Survival by age status, Class and sex, Deck, age and Sex"}
