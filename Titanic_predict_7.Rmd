...
### introducing interactions (no, not now)
...
```{r modelbuildrf3, cache=base_cache, dependson="dataclean1"}
# _______________________________
# Random Forests
#  
# parallel processing
...
set.seed(globalseed + 2)
modl <- modrf3 <- train(Survived ~ . , data = trainingset,
                        method = "rf",
                        # preProcess = "pca",
                        trControl = trainControl(method = "cv",
                                                 number = nfolds))
...
```{r modelbuildada, cache=base_cache, dependson="dataclean1", warning=FALSE}
...
set.seed(globalseed)
modl <- modada <- train(Survived ~ . , data = trainingset,
                        method = "adaboost",
                        #preProcess = "pca",
                        trControl = trainControl(method = "cv",
                                                 number = nfolds))
...
```{r modelbuildadarec, warning=FALSE}
# record attempt
cm <- confusionMatrix(data = predict( modl , newdata=validationset, type="raw"),
                reference = validationset$Survived)

recordmod$new(Name = "ADA", 
              Method = modl$method, 
              Preprocess = "None",
              Best.tune = paste0(colnames(modl$bestTune)[1], " = ", modl$bestTune[1]),
              Acc.CV = round(max(modl$results$Accuracy),3),
...
stk_train_df = data.frame(
        ...
        p_rf2 = predict(modrf2, newdata = trainingset, type="prob"),
        p_rf3 = predict(modrf3, newdata = trainingset, type="prob"),
        ...
        p_ada = predict(modada, newdata = trainingset, type="prob")
...
stk_valid_df = data.frame(
        ...
        p_rf2 = predict(modrf2, newdata = validationset, type="prob"),
        p_rf3 = predict(modrf3, newdata = validationset, type="prob"),
        ...
        p_ada = predict(modada, newdata = validationset, type="prob")
