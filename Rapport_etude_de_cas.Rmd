...
Les passagers de la 3è classe sont aussi les plus jeunes globalement.
...
```{r anx_age, fig.width=w.11, fig.asp=a.11}
pander(tb_age)

local({
        gr <- arrangeGrob(
                grobs = unlist(
                        list(p_age), # faire une liste de ggplots
                        list(p_age_sex), # faire une liste de ggplots
                        p_agestat_by), # est déjà une liste de ggplots
                ncol = 3)
        displaygraph(gr)
})
...
```{r anx_famly, fig.show = "hold", fig.width=w.11, fig.asp=a.11}
p_famly$famly
